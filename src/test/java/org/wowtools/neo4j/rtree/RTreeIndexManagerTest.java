...
public class RTreeIndexManagerTest {
    ...
    public void testEditIndex() {
        ...
        RTreeIndexManager.createIndex(db, "index1", "g", 64);
        ...
        RTreeIndexManager.createIndex(db, "index2", "g2", 64);
        RTreeIndex index2 = RTreeIndexManager.getIndex(db, "index2");
        assertEquals("index2", index2.getIndexName());
        System.out.println("get测试通过");

        RTreeIndex index3 = RTreeIndexManager.getOrCreateIndex(db, "index3", "g", 64);
        assertEquals("index3", index3.getIndexName());
        index2 = RTreeIndexManager.getOrCreateIndex(db, "index2", "gxx", 64);
        assertEquals("index2", index2.getIndexName());
        ...
        try {
            RTreeIndexManager.getIndex(db,"index1");
